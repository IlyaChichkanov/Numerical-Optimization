...
Вторая половина пары, 35–40 минут, — задачи, которые решаются руками. Ноутбук [`demo03.ipynb`](demo03.ipynb) на занятии не открываем (кроме одной ячейки с `marginals` в разделе 7): он для самостоятельной проверки тех же задач в коде после пары. Условия — в разделе 12, разбор с ответами — в [`exercises03.ipynb`](exercises03.ipynb).

| Мин | Задача | Что тренирует |
|-----|--------|---------------|
| 5 | **12.1** Завод с контрактом при разных $b$: $x^\ast$, $\mu^\ast$ из баланса сил, сравнение с $df^\ast/db$ | сила = цена; неактивное ограничение даёт $\mu=0$ |
| 7 | **12.6** Проекция точки на шар через множители: формула $x^\ast=p/(1+2\mu)$, два случая по знаку $\mu$ | одна кривая стенка, комплементарная нежёсткость, связь с проекцией лекции 2 |
| 7 | **12.7** Линейная цель на круге: $x^\ast=-c/\lVert c\rVert$, $\mu=\lVert c\rVert/2$; обсуждение — та же система на *внешности* круга | направление $\nabla h$ внутрь; система ККТ — необходимое условие, не достаточное |
| 8 | **12.8** LP по картинке: конусы нормалей в вершинах многоугольника, вершина-решение для $c=(5,1)$ и $c=(1,1)$ без симплекса, множители из системы $2\times2$ | активный набор по геометрии |
| 8 | **12.3** QP руками: угадать активный набор по безусловному минимуму, решить систему, проверить знаки $\mu$ | процедура активного набора — то, что делают методы части IV |

Дома: 12.9 (чувствительность — знак в «цене ограничения»), 12.10 (ящик наибольшего объёма — равенство в трёх переменных), 12.2, 12.4, 12.5; затем прогнать `demo03.ipynb`, части (a)–(d).
