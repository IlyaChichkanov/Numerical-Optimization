...
Ноутбук — для самостоятельной проверки задач семинара (раздел 12 конспекта) после пары: то, что решалось руками у доски, здесь считается кодом. Читается сверху вниз. Разминка и четыре части:
...
- **(b)** множители как баланс сил: $\mu=0.75$ для ящика прямо из $\nabla f(x^\ast)$; для LP планирования — разложение градиента прибыли по нормалям активных ограничений против `res.ineqlin.marginals`; задачи семинара 13.6–13.7 (шар и круг) в коде;
...
md(r"""Круг касается трёх сторон (1, 2, 3) — они активны, у них множители положительны; у сторон 4 и 5 зазор положителен и множители **нулевые**. Это комплементарная нежёсткость (раздел 5 конспекта) в чистом виде; а равенства $\sum_i\mu_i a_i=0$ и $\sum_i\mu_i\lVert a_i\rVert=1$ — баланс сил по $(c,r)$ (раздел 7, упражнение 12.4). Пока — просто наблюдение: «круг держат те стороны, которых он касается».""")


# ============================================================ (a) где живёт минимум
md(r"""## (a) Где живёт минимум: внутри или на границе

Ящик $[-1,1]^2$ и $f(x)=\tfrac12 x^\top Qx+c^\top x$, $Q=\begin{pmatrix}2&0.5\\0.5&1\end{pmatrix}$ (раздел 3 конспекта). Сравним **безусловный** минимум $\hat x=-Q^{-1}c$ с решением задачи с `bounds` для двух разных $c$: когда $\hat x$ внутри ящика, он и есть ответ; когда снаружи — решение на границе, и часть граней становится активной.""")
...
md(r"""**Несколько стенок — вершина LP** (раздел 5). Планирование производства: в вершине $(2,6)$ активны ограничения 2 и 3. Градиент прибыли $c=(3,5)$ раскладывается по их внешним нормалям $a_2=(0,2)$, $a_3=(3,2)$ — коэффициенты и есть множители, а `linprog` отдаёт их в `res.ineqlin.marginals` (со знаком минус для задачи на максимум).""")
...
md(r"""**Задачи семинара 13.6 и 13.7 в коде.** Проекция $p=(3,4)$ на единичный шар: формула $x^st=p/(1+2\mu)$, $\mu=	frac12(\lVert p
...
print("13.6 проекция: x* =", proj.round(4), " формула p/(1+2mu) =", (p / (1 + 2 * mu_proj)).round(4), " mu =", mu_proj)

c_lin = np.array([3.0, 4.0])
xs_lin = minimize(lambda x: c_lin @ x, x0=[0.1, 0.1], constraints=[{"type": "ineq", "fun": lambda x: 1 - x @ x}]).x
print("13.7 круг:     x* =", xs_lin.round(4), " формула -c/|c| =", (-c_lin / np.linalg.norm(c_lin)).round(4), " mu =", np.linalg.norm(c_lin) / 2)
print("баланс сил: c + mu * 2x =", (c_lin + np.linalg.norm(c_lin) / 2 * 2 * xs_lin).round(6))""")

# ============================================================ (c) равенство: касание
md(r"""## (c) Равенство — касание

Прямоугольник в круге (ДЗ 2, раздел 6 конспекта): $\min -4x_1x_2$ при $g(x)=x_1^2+x_2^2-1=0$, $x\ge0$. Решаем `minimize` с ограничением-равенством, затем проверяем касание: $\nabla f(x^\ast)=\lambda\nabla g(x^\ast)$, знак $\lambda$ любой.""")
...
md(r"""**Цена ограничения** (раздел 8). Меняем мощность второго цеха $b_2$ и смотрим на максимальную прибыль $f^\ast(b_2)$: кусочно-линейная функция, и её наклон около $b_2=12$ равен множителю $y_2^\ast=1.5$. За пределами участка набор активных ограничений меняется — и наклон тоже.""")
...
# ============================================================ (e) две прикладные задачи
md(r"""## (e) Две прикладные задачи: силы и цены

**Цепь на полу** (раздел 2.1 конспекта; модель — лекция 1, §7.3). Решаем с полом через `SLSQP` и `LinearConstraint`, затем читаем множители из баланса сил: у лежащего груза $\mu_i=\partial E/\partial z_i$ (реакция пола), у висящего $\nabla E=0$. Проверяем и $y$-компоненту: $\partial E/\partial y_i=-0.1\,\mu_i$ (пол наклонный, реакция перпендикулярна полу).""")
...
A_floor = np.hstack((-0.1 * np.eye(N), np.eye(N)))                       # h_i = z_i - 0.1 y_i - 0.5 >= 0
res_chain = minimize(energy, v0, jac=grad_E, method="SLSQP",
                     constraints=[LinearConstraint(A_floor, 0.5 * np.ones(N), np.inf)], options={"ftol": 1e-12, "maxiter": 500})
h = A_floor @ res_chain.x - 0.5
on_floor = h < 1e-6
G = grad_E(res_chain.x)
mu = np.zeros(N); mu[on_floor] = G[N:][on_floor]                          # dE/dz_i = mu_i для лежащих
print("грузов на полу:", on_floor.sum(), "из", N, " (номера", np.where(on_floor)[0][0] + 1, "-", np.where(on_floor)[0][-1] + 1, ")")
print("реакции пола mu_i, Н:", mu[on_floor].round(3))
print("сумма реакций", mu.sum().round(2), "Н; вес цепи", round(m * g * N, 2), "Н; остальное держат крепления")
print("проверка y-баланса dE/dy_i + 0.1 mu_i:", np.abs(G[:N][on_floor] + 0.1 * mu[on_floor]).max().round(8))
print("у висящих грузов |grad E| =", np.abs(np.r_[G[:N][~on_floor], G[N:][~on_floor]]).max().round(8), "-> пол не при чём")

y, z = unpack(res_chain.x)
plt.figure(figsize=(6.5, 3.4))
yy = np.linspace(-2.2, 2.2, 5); plt.plot(yy, 0.5 + 0.1 * yy, "k--", lw=1, label="пол")
plt.plot(y, z, "-", color=BLUE, lw=1.5)
plt.plot(y[1:-1][on_floor], z[1:-1][on_floor], "o", color=RED, ms=4, label="на полу: mu_i > 0")
plt.plot(y[1:-1][~on_floor], z[1:-1][~on_floor], "o", color=BLUE, ms=4, label="висят: mu_i = 0")
plt.legend(); plt.title("цепь на полу"); plt.show()""")

md(r"""**Портфель Марковица** (раздел 2.2). Четыре актива; $\min\tfrac12x^\top\Sigma x$ при $r^\top x\ge r_{\text{target}}$, $\mathbf 1^\top x=1$, $x\ge0$. Множители — из баланса сил по активному набору: $\Sigma x^\ast=\lambda\mathbf 1+\mu_r r+\sum_{i:\,x_i^\ast=0}\mu_i e_i$. Затем проверяем «цену доходности» конечной разностью: $d(\text{риск}^\ast)/d\,r_{\text{target}}=\mu_r$.""")

code("""r_exp = np.array([0.04, 0.08, 0.11, 0.15])
vol = np.array([0.05, 0.12, 0.18, 0.30])
corr = np.array([[1, 0.2, 0.1, 0.0], [0.2, 1, 0.5, 0.3], [0.1, 0.5, 1, 0.4], [0.0, 0.3, 0.4, 1.0]])
Sigma = np.outer(vol, vol) * corr

def portfolio(target):
    res = minimize(lambda x: 0.5 * x @ Sigma @ x, x0=np.ones(4) / 4, jac=lambda x: Sigma @ x, method="SLSQP", bounds=[(0, 1)] * 4,
                   constraints=[{"type": "eq", "fun": lambda x: x.sum() - 1}, {"type": "ineq", "fun": lambda x: r_exp @ x - target}],
                   options={"ftol": 1e-13, "maxiter": 500})
    return res.x, res.fun

x, risk = portfolio(0.12)
act0 = x < 1e-6
M = np.c_[np.ones(4), r_exp, np.eye(4)[:, act0]]                             # grad f = lam*1 + mu_r*r + sum mu_i e_i
sol = np.linalg.lstsq(M, Sigma @ x, rcond=None)[0]
lam, mu_r, mu_bounds = sol[0], sol[1], sol[2:]
print("доли x* =", x.round(4), " риск =", round(risk, 6), " доходность =", round(r_exp @ x, 4))
print("не куплены (граница x_i >= 0 активна):", np.where(act0)[0] + 1, " их множители:", mu_bounds.round(4))
print(f"lambda (бюджет) = {lam:.4f} (знак любой),  mu_r (доходность) = {mu_r:.4f}")
slope = (portfolio(0.1205)[1] - portfolio(0.1195)[1]) / 0.001
print(f"d(риск*)/d(цель) конечной разностью = {slope:.4f}  -> совпадает с mu_r: +1 п.п. доходности стоит +{mu_r / 100:.4f} риска")""")
...
| что означает множитель в задаче? | сила (цепь: ньютоны реакции пола) или цена (портфель: риск за процент доходности) — смотря что за задача | часть (e) |
